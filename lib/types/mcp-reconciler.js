...
import { commandExists } from "./mcp-servers.js";
...
export class McpReconciler {
    ...
    async probe(serverName) {
        ...
        if (config === undefined)
            throw new TypeError(`unknown MCP server "${serverName}"`);
        if (config.enabled === false)
            throw new TypeError(`MCP server "${serverName}" is disabled`);
        ...
        return {
            ...
            rollback: async () => {
                ...
                for (const [name, config] of previous) {
                    if (config.enabled === false || (config.transport === 'stdio' && !this.canResolveCommand(config.command ?? '')))
                        continue;
                    const fiber = this.ctx.plugin(mcpClient, clientConfig(config, this.pentestswarmApiKey));
                    await fiber.await();
                    this.fibers.set(name, fiber);
...
//# sourceMappingURL=mcp-reconciler.js.map