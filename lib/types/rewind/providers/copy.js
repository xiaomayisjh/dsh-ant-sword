...
function isExcluded(rel, excludeGlobs) {
    ...
    for (const pattern of excludeGlobs) {
        ...
        if (rel === pattern || rel.startsWith(pattern + sep))
            return true;
    }
    return false;
}
...
async function* walkFiles(root, excludeGlobs, dir = root) {
    ...
    for (const entry of entries) {
        ...
        if (rel === '' || isExcluded(rel, excludeGlobs))
            continue;
        if (entry.isDirectory())
            yield* walkFiles(root, excludeGlobs, path);
        else if (entry.isFile())
            yield path;
    }
}
/** Create the copy provider bound to a snapshot root and exclusion segments. */
export function makeCopyProvider(snapshotDir, excludeGlobs) {
    return {
        kind: 'copy',
        ...
        async capture(cwd, signal) {
            ...
            for await (const file of walkFiles(cwd, excludeGlobs)) {
                ...
                if (await hasSymlinkInChain(cwd, file))
                    continue;
                const rel = relative(cwd, file);
                const dest = join(target, rel);
                if (relative(target, dest).startsWith('..'))
                    continue;
        ...
        async restore(cwd, ref, signal) {
            ...
            for await (const file of walkFiles(source, [])) {
                if (signal?.aborted === true)
                    throw new Error('snapshot restore aborted');
                if (await hasSymlinkInChain(source, file))
                    continue;
                const rel = relative(source, file);
                const dest = join(cwd, rel);
                if (relative(cwd, dest).startsWith('..'))
                    continue;
                if (await hasSymlinkInChain(cwd, dest))
                    continue;
                await mkdir(resolve(dest, '..'), { recursive: true });
                try {
                    await writeFile(dest, await readFile(file));
