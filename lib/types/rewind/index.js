...
import { SnapshotProviderRegistry } from "./registry.js";
import { rewindDomain } from "./domain.js";
const DEFAULT_MUTATION_TOOLS = ['bash', 'write', 'edit', 'str_replace_editor', 'pwsh', 'terminal_send'];
const DEFAULT_EXCLUDE_GLOBS = ['node_modules', '.git', '.dsh', 'dist', 'build'];
/** Schemastery validation for {@link RewindPluginConfig}. */
export const RewindConfigSchema = z.object({
    enabled: z.boolean(),
    provider: z.union(['auto', 'git', 'copy']),
    gitBin: z.string(),
    ...
    listLimit: z.number(),
    preRewindCheckpoint: z.union(['warn', 'require', 'off']),
});
...
function formatList(records, limit) {
    if (records.length === 0)
        return 'ant-sword rewind: no checkpoints yet';
...
function resolveTarget(records, input) {
    ...
    if (stepMatch?.[1] !== undefined) {
        ...
        for (const record of records) {
            ...
            if (step === undefined || step > target)
                continue;
            if (best === undefined || step > (best.step ?? -1))
                best = record;
        }
        return best;
    }
...
export function applyRewind(ctx, config) {
    ...
    async function prune(domain, sessionId) {
        ...
        if (total <= resolved.maxSnapshotBytes)
            return;
