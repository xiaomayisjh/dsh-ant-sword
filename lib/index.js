...
function applyRuntimeStatus(ctx, getServers) {
	let disposed = false;
	let running = false;
	let latest = {
		checkedAt: Date.now(),
...
function applyInstallApi(ctx) {
	...
	ctx.webServer.register({
		...
		handler: (_req, res) => sendJson$1(res, 200, {
...
function applyDynamicRuntime(ctx, mcpServers, pentestswarmApiKey, skillsReconciler = new SkillsReconciler()) {
	...
	const controller = new RuntimeController(ctx.settings.register(settingsNamespace(ANT_SWORD_SETTINGS_NAMESPACE), AntSwordRuntimeConfigSchema, {
		base,
		applies: "live",
		validate: validateRuntimeConfig
	}), [
		new McpReconciler(ctx, pentestswarmApiKey),
		skillsReconciler,
...
function apply(ctx, config) {
	...
	const controller = applyDynamicRuntime(ctx, config.mcpServers === void 0 || config.mcpServers.length === 0 ? DEFAULT_MCP_SERVERS : config.mcpServers, config.pentestswarmApiKey, skillsReconciler);
