...
function distinctReasoningFromEfforts(efforts) {
  ...
  for (const id of ordered) {
    ...
    seen.add(id);
    distinct.push({ id, name: effortLabel(id) });
  }
...
var ThinkingPolicyRuntime = class {
   ...
   */
  syntheticReasoningFor(providerId, modelId) {
    ...
    const efforts = explicit !== void 0 ? explicit.simulatedEfforts : applied.defaultThinkingFallback === void 0 ? DEFAULT_THINKING_FALLBACK : applied.defaultThinkingFallback;
    if (efforts === null) return void 0;
    const distinct = distinctReasoningFromEfforts(efforts);
    return { efforts: distinct.efforts, defaultEffort: distinct.defaultEffort };
  }
  install(agent) {
