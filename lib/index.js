...
async function readJson(req) {
	...
	for await (const chunk of req) {
		...
		if (size > MAX_BODY_BYTES$2) throw new TypeError(`request body exceeds ${String(MAX_BODY_BYTES$2)} bytes`);
		chunks.push(bytes);
	}
	return JSON.parse(Buffer.concat(chunks).toString("utf8"));
}
...
function optionalRevision(value) {
	...
	if (!Number.isSafeInteger(value) || value < 0) throw new TypeError("expectedRevision must be a non-negative safe integer");
	return value;
}
function parseRuntimeConfigMutation(value) {
	...
	if (typeof value.field !== "string" || !MUTABLE_FIELDS.has(value.field)) throw new TypeError("field must be one of mcpServers, disabledSkills, or rules");
	const allowed = value.op === "set" ? new Set([
		"op",
		"field",
		"value",
		"expectedRevision"
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			if (!isLoopbackRequest(req)) {
				sendJson$2(res, 403, { error: "loopback-only" });
				return;
			}
			if (req.method === "GET") {
				try {
					sendJson$2(res, 200, runtimeConfigApiView(ctx.settings, controller));
				} catch (error) {
					sendJson$2(res, 503, { error: error instanceof Error ? error.message : String(error) });
				}
				return;
			}
			if (req.method !== "POST") {
			...
			try {
				...
				sendJson$2(res, 200, await mutateRuntimeConfig(ctx.settings, controller, mutation));
			} catch (error) {
				sendJson$2(res, error instanceof SettingsConflictError ? 409 : error instanceof TypeError ? 400 : 500, { error: error instanceof Error ? error.message : String(error) });
			}
		}
	});
}
