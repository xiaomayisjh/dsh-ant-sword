...
name: pwn-chain
description: |
  从漏洞点到可验证利用 (Working Exploit) 的全链路工程化方法。
  适用场景：拿到了二进制 + 漏洞点 + 目标环境，需要构建一个能稳定拿 shell 的 exploit（不是只能本地触发、远程一打就崩的脚本）。
  覆盖三大方向：栈溢出 / 堆利用 / 内核 pwn。强调"CTF 本地通 → 远程稳定拿 flag"的工程差距：libc 版本错配、堆喷射时序、SMEP/SMAP/KASLR、栈对齐、远程缓冲。
...
6. `ALWAYS`: 遵守渐进式推理纪律（`../../references/progressive-reasoning.md`）——利用构造按 crash→control→leak→primitive→chain→verify 逐级推进，每级引用上一级的 Observed 事实
...
3. **真实目标的二进制利用** — SRC / 红队场景下，已经识别到内存损坏漏洞，需要构造完整 exploit
...
| 场景 | 用什么 |
|------|--------|
| 识别 custom VM / anti-debug / 复杂 obfuscation | `reverse-master` 技能（回退 `../sec-reverse/`） |
| 从零打开二进制做静态分析 | `reverse-master` 技能（回退 `../sec-reverse/`） |
| **有漏洞点，写 exploit 打通远程** | **本 skill** |
| 把 pwn 拿到的 shell 整合进完整攻击链 | `attack-chain/`（下游） |

`reverse-master` 技能（回退 `../sec-reverse/`） 关注"理解程序在干什么"（模式识别、协议还原、解 CTF 题里的奇怪机制）；本 skill 关注"把已经看懂的漏洞变成可执行的攻击"。两者经常配套使用，但分工清晰。
...
- 还没看懂二进制在干什么 → `reverse-master` 技能（回退 `../sec-reverse/`）
- 需要静态详细分析 → `reverse-master` 技能
- 快速侦察确认架构/保护机制 → `reverse-master` 技能
