...
/** One preset file's relative path and content. */
interface PresetFile {
  readonly rel: string
  readonly content: Buffer
}

async function readPresetFiles(dir: string, prefix = ''): Promise<PresetFile[]> {
  let entries
  try {
 ...
 */
async function syncPreset(source: string, id: string): Promise<string> {
  ...
  for (const file of files) {
    ...
    if (existing !== undefined && existing.equals(file.content)) continue
...
export async function syncRedTeamPreset(): Promise<string> {
  return syncPreset(PRESET_SOURCE, RED_TEAM_PRESET_ID)
}

/** Sync the autonomous red-team-auto preset. */
export async function syncRedTeamAutoPreset(): Promise<string> {
