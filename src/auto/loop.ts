...
interface LoopState {
  startedAt: number
  /** Recent Intent signatures for the stall detector, newest last. */
...
export function applyAutoLoop(ctx: Context, config: AutoLoopConfig): void {
  ...
  const stateOf = (sessionId: string): LoopState => {
  ...
  ctx.tools.register(defineTool({
    name: 'board_read',
    description:
      'Read the current blackboard: every Fact, open Intent, Hint, and the Goal, with the loop cycle and pause/complete flags. '
    ...
    async execute(_args, exec) {
      ...
      const lines = snap.nodes.map(n =>
        `#${n.id} [${n.kind}${n.status !== undefined ? `/${n.status}` : ''}] (cycle ${n.cycle}) ${n.label}${n.parentId !== undefined ? ` <- ${n.parentId}` : ''}`)
      return {
        summary: [
  ...
  ctx.tools.register(defineTool({
    name: 'board_complete',
    description:
      'Mark the engagement goal reached. Call only when you hold evidence the Goal node is satisfied; this stops the autonomous loop.',
    parameters: {
  ...
  ctx.on('agent/status', ({ agent, status }) => {
    ...
    void (async () => {
      ...
      if (snap.nodes.length === 0) return // not bootstrapped; wait for first operator prompt
