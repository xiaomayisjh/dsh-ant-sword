...
function assertHexRef(ref: string): void {
  if (!HEX.test(ref)) {
...
async function git(gitBin: string, cwd: string, args: readonly string[], signal?: AbortSignal): Promise<string> {
...
async function probe(gitBin: string, cwd: string, signal?: AbortSignal): Promise<boolean> {
  try {
...
async function capture(gitBin: string, cwd: string, signal?: AbortSignal): Promise<CaptureResult> {
...
async function trackedPaths(gitBin: string, cwd: string, ref: string, signal?: AbortSignal): Promise<string[]> {
  assertHexRef(ref)
  const listing = await git(gitBin, cwd, ['ls-tree', '-r', '--name-only', ref], signal)
...
export function makeGitProvider(gitBin: string): SnapshotProvider {
  return {
    kind: 'git',
